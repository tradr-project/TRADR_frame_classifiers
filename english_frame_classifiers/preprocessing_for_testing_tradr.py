...
flag = sys.argv[1] # e.g., 'tradr_test_a'
data_output = sys.argv[2]

if flag.startswith('tradr'):
  # Read in TRADR testing data
  df_tradr_test = pd.read_csv("./data/"+flag+".tsv", delimiter='\t', header=0, names=['speaker', 'sentence', 'frame', 'parent', 'lu', 'target', 'da'], quoting=3)
  prefix = flag+"_"
elif flag == 'framenet':
  # Read in FrameNet for sampling data
  df_tradr_test = pd.read_csv("./data/framenet_for sampling.tsv", delimiter='\t', header=0, names=['sentence_text', 'target_indices', 'lu', 'frame'], quoting=3)
  prefix = "framenet_for_sampling_"
else:
  print("Valid flags: 'tradr_test_a', 'tradr_test_b', 'tradr_test_c', 'framenet'.")

# Read in TRADR main data (used for reference)
df_tradr_main = pd.read_csv("./data/tradr_main.tsv", delimiter='\t', header=0, names=['speaker', 'sentence', 'frame', 'parent', 'lu', 'target', 'da'], quoting=3)
...
# Speaker and dialogue act tags are available only for TRADR data
if flag.startswith('tradr'):
  # Extract speakers and dialogue acts and convert to numbers
  speaker_make = LabelEncoder()
  numerical_speakers = speaker_make.fit_transform(df_tradr_main.speaker.values)

  da_make = LabelEncoder()
  numerical_das = da_make.fit_transform(df_tradr_main.da.values)

  # Create two reference dictionaries: speaker --> numerical_value and da --> numerical value
  ref_dict_speaker = dict(zip(df_tradr_main.speaker.values, numerical_speakers))
  ref_dict_da = dict(zip(df_tradr_main.da.values, numerical_das))

  # Map speakers and dialogue acts of data being pre-processed to numerical values
  numerical_speakers_df = [ref_dict_speaker[s] for s in df_tradr_test.speaker.values]
  speaker_tensors = torch.tensor(numerical_speakers_df).long()

  numerical_das_df = [ref_dict_da[d] for d in df_tradr_test.da.values]
  da_tensors = torch.tensor(numerical_das_df).long()

lus = df_tradr_main.lu.values
...
pairs = zip(lus, df_tradr_main.frame.values)
...
lus_test = df_tradr_test.lu.values

filtering_masks = [create_frame_filtering_mask(lu_frames[lu], df_tradr_main.frame.nunique()) for lu in lus_test]
...
target_indices = df_tradr_test.target_indices.values
...
sentences = df_tradr_test.sentence_text.values
sentences_marked = []
...
# Set the maximum sentence length (for TRADR it's 45)
MAX_LEN = 45
...
# Save everything
if flag.startswith('tradr'):
  data_output = "./test_data/tradr/"+prefix # e.g., "./test_data/tradr/tradr_test_a_"
elif flag == 'framenet':
  data_output = "./train_data/tradr/"+prefix # "./train_data/tradr/framenet_for_sampling_"

torch.save(label_tensors, data_output+"label_tensors_class_81_len_45.pt")
torch.save(text_tensors, data_output+"text_tensors_class_81_len_45.pt")
torch.save(attention_tensors, data_output+"attention_tensors_class_81_len_45.pt")
torch.save(position_tensors, data_output+"position_tensors_class_81_len_45.pt")
torch.save(beta_tensors, data_output+"beta_tensors_class_81_len_45.pt")
torch.save(filtering_masks_tensors, data_output+"filtering_masks_tensors_class_81_len_45.pt")
torch.save(pos_tags_tensors, data_output+"pos_tags_tensors_class_81_len_45.pt")
torch.save(subword_tensors, data_output+"subword_tensors_class_81_len_45.pt")
if flag.startswith('tradr'):
  torch.save(speaker_tensors, data_output+"speaker_tensors_class_81_len_45.pt")
  torch.save(da_tensors, data_output+"da_tensors_class_81_len_45.pt")
