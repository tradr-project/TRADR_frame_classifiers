...
test_speaker = torch.load("../test_data/tradr/"+test_name+"_speaker_tensors_class_81_len_45.pt")
test_da = torch.load("../test_data/tradr/"+test_name+"_da_tensors_class_81_len_45.pt")
...
class BertFrameClassifier(torch.nn.Module):
  def __init__(self, num_labels, dropout=0.1):
    ...
    self.linear_1 = torch.nn.Linear(1542, 768)
    self.tanh = torch.nn.Tanh()
    self.linear_2 = torch.nn.Linear(768, num_labels)
    self.embed_speaker = torch.nn.Embedding(3, 2)
    self.embed_da = torch.nn.Embedding(12, 4)
  ...
  def forward(self, text_tensors_batch, attention_tensors_batch, position_tensors_batch, beta_tensors_batch, speaker_tensors_batch, da_tensors_batch):
    ...
    looked_up_speaker = self.embed_speaker(speaker_tensors_batch) # B x L x 4
    # Convert dialogue act tags to embeddings
    looked_up_da = self.embed_da(da_tensors_batch) # B x L x 1
...
checkpoint = torch.load("../models/w10_hl12_speaker_da.pt", map_location="cuda:3")
...
for batch in pbar(test_dataloader):
  ...
  input_speaker_b = input_speaker_b.to('cuda:3')
  input_da_b = input_da_b.to('cuda:3')
...
with open("../w10_hl12_speaker_da_"+test_name+"_wrong_predictions.txt", "w") as f:
