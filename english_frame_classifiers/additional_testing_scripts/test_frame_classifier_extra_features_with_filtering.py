...
test_filter = torch.load("../test_data/tradr/"+test_name+"_filtering_masks_tensors_class_81_len_45.pt")
...
class BertFrameClassifier(torch.nn.Module):
  def __init__(self, num_labels, dropout=0.1):
    ...
    self.linear_1 = torch.nn.Linear(1546, 773)
  ...
  def forward(self, text_tensors_batch, attention_tensors_batch, position_tensors_batch, beta_tensors_batch, pos_tags_tensors_batch, subword_tensors_batch, filtering_tensors_batch):
    ...
    linear_output_2 = torch.mul(linear_output_2, filtering_tensors_batch.float())
    return linear_output_2

frame_clf = BertFrameClassifier(num_labels=num_frames)

checkpoint = torch.load("../models/w10_hl12_pos_sub_filter.pt", map_location="cuda:3")
...
with open("../w10_hl12_pos_sub_filter_"+test_name+"_wrong_predictions.txt", "w") as f:
