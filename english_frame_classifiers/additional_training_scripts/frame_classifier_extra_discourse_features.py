...
# The model uses information about targets or context, as well as additional discourse features, such as
# speaker and dialogue act tags of the utterances. The model learns to differentiate between 81 frame labels. 
...
class BertFrameClassifier(torch.nn.Module):
  def __init__(self, num_labels, dropout=0.1):
    ...
    self.linear_1 = torch.nn.Linear(1542, 768).to('cuda:2')
    self.tanh = torch.nn.Tanh()
    self.linear_2 = torch.nn.Linear(768, num_labels).to('cuda:3')
    self.embed_speaker = torch.nn.Embedding(3, 2).to('cuda:1')
    self.embed_da = torch.nn.Embedding(12, 4).to('cuda:1')
    ...
    torch.nn.init.xavier_uniform_(self.embed_speaker.weight)
    torch.nn.init.xavier_uniform_(self.embed_da.weight)

  def forward(self, text_tensors_batch, attention_tensors_batch, position_tensors_batch, beta_tensors_batch, speaker_tensors_batch, da_tensors_batch):
    # Convert speaker tags to embeddings
    looked_up_speaker = self.embed_speaker(speaker_tensors_batch) # B x 1 x 2
    # Convert dialogue act tags to embeddings
    looked_up_da = self.embed_da(da_tensors_batch) # B x 1 x 4
    ...
    # Concatenate input tensors with speaker embeddings
    input_tensors_batch = torch.cat((input_tensors_batch, looked_up_da, looked_up_speaker), 1)
...
speaker_tensors = torch.load("../train_data/tradr/tradr_speaker_tensors_class_81_len_45.pt")
da_tensors = torch.load("../train_data/tradr/tradr_da_tensors_class_81_len_45.pt")
...
for train_idx, test_idx in skf.split(text_tensors, label_tensors):
  ...
  train_speaker, test_speaker = speaker_tensors[train_idx], speaker_tensors[test_idx]
  train_da, test_da = da_tensors[train_idx], da_tensors[test_idx]

  # Select a batch size for loading tensors.
  batch_size = 8

  # Create an iterator of our data with torch DataLoader.
  train = TensorDataset(train_data, train_att, train_pos, train_beta, train_speaker, train_da, train_labels)
  train_sampler = RandomSampler(train)
  train_dataloader = DataLoader(train, sampler=train_sampler, batch_size=batch_size)
  
  test = TensorDataset(test_data, test_att, test_pos, test_beta, test_speaker, test_da, test_labels)
  ...
  for epoch_i in range(epochs):
    ...
    for batch in pbar(train_dataloader):
      ...
      input_ids_b, input_att_b, input_pos_b, input_beta_b, input_speaker_b, input_da_b, labels_b = batch
      ...
      input_da_b = input_da_b.to('cuda:1')
      labels_b = labels_b.to('cuda:3')
      # Clear out the gradients (by default they accumulate)
      optimizer.zero_grad()
      # Forward pass
      labels_b_pred = frame_clf(input_ids_b, input_att_b, input_pos_b, input_beta_b, input_speaker_b, input_da_b) 
  ...
  for batch in test_dataloader:
    ...
    with torch.no_grad():
      ...
      logits = frame_clf(input_ids_b, input_att_b, input_pos_b, input_beta_b, input_speaker_b, input_da_b)
  ...
  if model_iba > best_iba:
    ...
    torch.save(checkpoint, "../models/w10_hl12_speaker_da.pt")
