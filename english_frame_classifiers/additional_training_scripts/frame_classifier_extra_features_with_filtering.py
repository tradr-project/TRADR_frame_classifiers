...
class BertFrameClassifier(torch.nn.Module):
  def __init__(self, num_labels, dropout=0.1):
    ...
    #self.linear_3 = torch.nn.Linear(num_labels, num_labels).to('cuda:3')
    ...
    #torch.nn.init.constant_(self.linear_3.bias, 0)
    torch.nn.init.xavier_uniform_(self.embed_pos.weight)
    torch.nn.init.xavier_uniform_(self.embed_subword.weight)

  def forward(self, text_tensors_batch, attention_tensors_batch, position_tensors_batch, beta_tensors_batch, pos_tags_tensors_batch, subword_tensors_batch, filtering_tensors_batch):
    # Convert POS tags to embeddings
    looked_up_pos = self.embed_pos(pos_tags_tensors_batch) # B x L x 768
    # Convert subword tags to embeddings
    looked_up_subword = self.embed_subword(subword_tensors_batch) # B x L x 768
    ...
    last_hidden_state = torch.cat((last_hidden_state, looked_up_pos, looked_up_subword),2)
    # Dropout
    ...
    linear_output_2 = torch.mul(linear_output_2, filtering_tensors_batch.float()).to('cuda:3')
    #linear_output_3 = self.linear_3(linear_output_2)
...
filtering_tensors = torch.load("../train_data/tradr/tradr_filtering_masks_tensors_class_81_len_45.pt")
...
for train_idx, test_idx in skf.split(text_tensors, label_tensors):
  ...
  train_filter, test_filter = filtering_tensors[train_idx], filtering_tensors[test_idx]

  # Select a batch size for loading tensors.
  batch_size = 8

  # Create an iterator of our data with torch DataLoader.
  train = TensorDataset(train_data, train_att, train_pos, train_beta, train_pos_tags, train_sub, train_filter, train_labels)
  train_sampler = RandomSampler(train)
  train_dataloader = DataLoader(train, sampler=train_sampler, batch_size=batch_size)
  
  test = TensorDataset(test_data, test_att, test_pos, test_beta, test_pos_tags, test_sub, test_filter, test_labels)
  ...
  for epoch_i in range(epochs):
    ...
    for batch in pbar(train_dataloader):
      ...
      input_ids_b, input_att_b, input_pos_b, input_beta_b, input_pos_tags_b, input_sub_b, input_filter_b, labels_b = batch
      ...
      input_filter_b = input_filter_b.to('cuda:3')
      labels_b = labels_b.to('cuda:3')
      # Clear out the gradients (by default they accumulate)
      optimizer.zero_grad()
      # Forward pass
      labels_b_pred = frame_clf(input_ids_b, input_att_b, input_pos_b, input_beta_b, input_pos_tags_b, input_sub_b, input_filter_b) 
  ...
  for batch in test_dataloader:
    ...
    with torch.no_grad():
      ...
      logits = frame_clf(input_ids_b, input_att_b, input_pos_b, input_beta_b, input_pos_tags_b, input_sub_b, input_filter_b)
  ...
  if model_iba > best_iba:
    ...
    torch.save(checkpoint, "../models/w10_hl12_pos_sub_filter.pt")
