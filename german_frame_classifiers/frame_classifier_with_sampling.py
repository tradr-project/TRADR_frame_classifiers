...
class BertFrameClassifier(torch.nn.Module):
  def __init__(self, num_labels, dropout=0.1):
    ...
    self.model = AutoModel.from_pretrained('bert-base-german-dbmdz-cased', output_hidden_states=False, output_attentions=False).to('cuda:1')
...
# Load data
label_tensors = torch.load("./train_data/tradr_main_label_tensors_class_72_len_67.pt")
text_tensors= torch.load("./train_data/tradr_main_text_tensors_class_72_len_67.pt")
attention_tensors = torch.load("./train_data/tradr_main_attention_tensors_class_72_len_67.pt")
position_tensors = torch.load("./train_data/tradr_main_position_tensors_class_72_len_67.pt")
beta_tensors = torch.load("./train_data/tradr_main_beta_tensors_class_72_len_67.pt")

# Load SALSA data for sampling
label_tensors_salsa = torch.load("./train_data/salsa_for_sampling_label_tensors_class_72_len_67.pt")
text_tensors_salsa = torch.load("./train_data/salsa_for_sampling_text_tensors_class_72_len_67.pt")
attention_tensors_salsa = torch.load("./train_data/salsa_for_sampling_attention_tensors_class_72_len_67.pt")
position_tensors_salsa = torch.load("./train_data/salsa_for_sampling_position_tensors_class_72_len_67.pt")
beta_tensors_salsa = torch.load("./train_data/salsa_for_sampling_beta_tensors_class_72_len_67.pt")

# Sample from SALSA data
...
# Function to perform sampling from a certain class
def sample_from_class(frame_numeric, n_samples):
  # get a subset of loaded SALSA data
  idx = torch.nonzero(label_tensors_salsa == frame_numeric, as_tuple=True)[0] # a list of indices
  label_sample = label_tensors_salsa[idx]
  text_sample = text_tensors_salsa[idx]
  attention_sample = attention_tensors_salsa[idx]
  position_sample = position_tensors_salsa[idx]
  beta_sample = beta_tensors_salsa[idx]
  if len(idx) > n_samples:
    # perform random sampling from SALSA
...
print('Sampled', label_sample.shape[0], 'instances of', label_tensors_salsa.shape[0])

# Split sampled SALSA fraction into 5 parts
# to be used for training in each of 5 folds
...
for tradr_idx, salsa_idx in zip(skf.split(text_tensors, label_tensors), kf.split(text_sample, label_sample)):
  ...
  # Concatenate a part of TRADR data with a part of sampled SALSA data
  train_data = torch.cat((train_data, text_sample[salsa_idx[0]]), dim=0)
  train_att = torch.cat((train_att, attention_sample[salsa_idx[0]]), dim=0)
  train_labels = torch.cat((train_labels, label_sample[salsa_idx[0]]), dim=0)
  train_pos = torch.cat((train_pos, position_sample[salsa_idx[0]]), dim=0)
  train_beta = torch.cat((train_beta, beta_sample[salsa_idx[0]]), dim=0)
  ...
  frame_clf = BertFrameClassifier(num_labels=72)
  ...
  if model_iba > best_iba:
    ...
    torch.save(checkpoint, "./models/w10_hl12_sampled_checkpoint.pt")
